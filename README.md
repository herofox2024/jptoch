...
- **术语任务独立进度**：术语提取和译后术语统一在「术语表 -> 术语任务」中执行，并使用独立进度条，不占用状态监控页的翻译进度。
...
6. 如需固定专有名词，进入「术语表」页的「术语任务」，选择原始 EPUB 并提取术语。
7. 在「术语任务」中勾选需要使用的 profile；术语提取进度只显示在术语任务区域。
8. 回到「任务」页，勾选启用术语表；工作台只显示当前已选 profile 摘要，详细管理统一在术语表页完成。
9. 点击开始翻译，在「状态」页查看翻译进度、预计剩余时间、速度、API 次数和校对详情。
10. 翻译完成后，用 EPUB 阅读器检查目录、章节跳转和排版。
...
- 选择一册或多册原始 EPUB，独立提取并生成 book profile。
- 勾选翻译时要注入的 profile；工作台只保留启用开关和已选数量摘要。
- 选择已翻译 EPUB，使用 profile 的中文别名统一术语，并输出 `_glossary_fixed.epub` 副本。
- 提取术语和译后统一使用术语任务自己的进度条；不会伪装成状态监控中的翻译进度。
...
- 模型缓存、跨模型文本缓存和人工译文统一存储在 SQLite，使用 WAL 和索引查询，启动时不再全量载入内存。
- 首次启动会自动迁移 `cache.json`、`text_cache.json`、`manual_cache.json`；迁移幂等，原 JSON 文件保留且不会自动删除。
- 普通缓存采用批量事务写入；人工译文立即持久化。两年以上未使用的普通缓存会自动清理，人工译文和已验证文本缓存永久保留。
...
- 当前书籍清理缓存会按原文哈希直接删除 SQLite 记录；选择跨模型清理时，会同时删除旧版明文 key、哈希 key 和不同模型下的同源文本缓存。
...
术语提取使用 `glossaryBookExtractionProgressChanged` 更新术语任务进度条；翻译使用 `progressChanged` 更新状态监控进度。`operationType` 用于避免术语提取或译后统一期间显示翻译进度。
...
其中，`qml_smoke_test.py` 是 QML 冒烟测试：它以较低成本检查 QML 页面和组件能否被读取、解析和加载，适合在修改 UI 后快速发现明显的语法或引用错误。它不能替代完整功能测试；发布前仍应运行 `pytest` 和真实 EPUB 回归测试。
